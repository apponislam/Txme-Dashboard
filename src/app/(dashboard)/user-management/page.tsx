...
//                                             <div className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-sm font-medium min-w-25 ${user.status === "active" ? "text-[#166534] bg-[#DCFCE7]" : user.status === "pending" ? "text-[#EAB308] bg-[#FEF9C3]" : user.status === "suspend" ? "text-[#991B1B] bg-[#FECACA]" : "text-[#6B7280] bg-[#E5E7EB]"}`}>
...
import { Search, Eye, MoreVertical, ChevronLeft, ChevronRight, Loader2 } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Card, CardContent } from "@/components/ui/card";
...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { UserModal } from "@/components/dashboard/user-management/UserDetails";
import { useGetUsersQuery, useUpdateUserStatusMutation, useDeleteUserMutation } from "@/redux/features/user/userApi";
...
const UserManagementPage = () => {
    ...
    const pagination = data?.pagination || { total: 0, limit: usersPerPage, page: 1, totalPage: 1 };

    const handleRoleChange = (role: "all" | "CUSTOMER" | "PROVIDER") => {
        setSelectedRole(role);
        setCurrentPage(1); // Reset to first page when filter changes
    };

    const handleStatusChange = (status: "all" | "active" | "pending" | "suspended" | "rejected") => {
        setSelectedStatus(status);
        setCurrentPage(1); // Reset to first page when filter changes
    };

    const handleUpdateStatus = async (userId: string, status: "active" | "rejected" | "suspended") => {
        try {
        ...
        return (
            <div className="space-y-6">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative w-1/6">
                        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#64748B] h-4 w-4 z-10" />
                        <Input disabled value="" className="pl-10 bg-[#E5E7EB] border-0" />
                    </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
                <div className="relative w-1/6">
                    ...
                    <Input
                        ...
                        className="pl-10 bg-[#E5E7EB] border-0 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#FF5A36] focus:ring-offset-0 transition-all"
                    />
                </div>

                <Select value={selectedRole} onValueChange={handleRoleChange} defaultValue="all">
                    <SelectTrigger className="border-[#D1D5DB]">
                        <SelectValue>{selectedRole === "all" ? "Types" : selectedRole === "CUSTOMER" ? "Customer" : "Provider"}</SelectValue>
                    </SelectTrigger>
                    ...
                    <SelectContent>
                        <div className="flex flex-col p-2 gap-3">
                            ...
                            <SelectItem value="CUSTOMER" className="flex items-center justify-center rounded-full px-4 py-2 text-[#166534] bg-[#DCFCE7] font-medium text-sm data-highlighted:bg-[#DCFCE7] data-state-checked:bg-[#DCFCE7] [&>span]:hidden">
                                <Image src="/dashboard/user-management/usericon1.svg" alt="user icon " height={12} width={12} />
                                Customer
                            </SelectItem>

                            <SelectItem value="PROVIDER" className="flex items-center justify-center rounded-full px-4 py-2 text-[#EAB308] bg-[#FEF9C3] font-medium text-sm data-highlighted:bg-[#FEF9C3] data-state-checked:bg-[#FEF9C3] [&>span]:hidden">
                                <Image src="/dashboard/user-management/usericon2.svg" alt="user icon " height={12} width={12} />
                                Provider
                            </SelectItem>
                        </div>
                    </SelectContent>
                ...
                <Select value={selectedStatus} onValueChange={handleStatusChange} defaultValue="all">
                    <SelectTrigger className="border-[#D1D5DB]">
                        <SelectValue>{selectedStatus === "all" ? "Status" : selectedStatus === "active" ? "Active" : selectedStatus === "pending" ? "Pending" : selectedStatus === "suspended" ? "Suspended" : "Rejected"}</SelectValue>
                    </SelectTrigger>
                    ...
                    <SelectContent>
                        <div className="flex flex-col p-2 gap-3">
                            ...
                            {/* Active */}
                            <SelectItem value="active" className="flex items-center justify-center rounded-full px-4 py-2 text-[#166534] bg-[#DCFCE7] font-medium text-sm data-highlighted:bg-[#DCFCE7] data-state-checked:bg-[#DCFCE7] [&>span]:hidden">
                                <span className="w-2 h-2 bg-[#22C55E] rounded-full"></span>
                                Active
                            </SelectItem>

                            {/* Pending */}
                            <SelectItem value="pending" className="flex items-center justify-center rounded-full px-4 py-2 text-[#EAB308] bg-[#FEF9C3] font-medium text-sm data-highlighted:bg-[#FEF9C3] data-state-checked:bg-[#FEF9C3] [&>span]:hidden">
                                <span className="w-2 h-2 bg-[#EAB308] rounded-full"></span>
                                Pending
                            </SelectItem>

                            {/* Suspended */}
                            <SelectItem value="suspended" className="flex items-center justify-center rounded-full px-4 py-2 text-[#991B1B] bg-[#FECACA] font-medium text-sm data-highlighted:bg-[#FECACA] data-state-checked:bg-[#FECACA] [&>span]:hidden">
                                <span className="w-2 h-2 bg-[#EF4444] rounded-full"></span>
                                Suspended
                            </SelectItem>

                            {/* Rejected */}
                            <SelectItem value="rejected" className="flex items-center justify-center rounded-full px-4 py-2 text-[#6B7280] bg-[#E5E7EB] font-medium text-sm data-highlighted:bg-[#E5E7EB] data-state-checked:bg-[#E5E7EB] [&>span]:hidden">
                                <span className="w-2 h-2 bg-[#6B7280] rounded-full"></span>
                                Rejected
                            </SelectItem>
                        </div>
            ...
            <Card className="border-[#E2E8F0] shadow-[0px_1px_16px_0px_rgba(0,0,0,0.08)] p-0">
                <CardContent className="p-0">
                    <Table>
                        ...
                        <TableBody>
                            {users.map((user: any) => (
                                <TableRow key={user._id} className="border-[#E2E8F0] hover:bg-[#F9FAFB]">
                                    {/* User Column */}
                                    <TableCell>
                                        <div className="flex items-center gap-3 p-2">
                                            <div className="relative h-10 w-10">
                                                {user.profilePicture ? (
                                                    <Image src={user.profilePicture} alt={user.fullName || user.email} fill className="rounded-full object-cover" sizes="40px" />
                                                ) : (
                                                    <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                                                        <span className="text-sm font-medium text-gray-600">{(user.fullName || user.email).charAt(0).toUpperCase()}</span>
                                                    </div>
                                                )}
                                            </div>
                                            <div>
                                                <div className="text-[#1E293B] font-medium">{user.fullName || "No Name"}</div>
                                                <div className="text-[#64748B] text-sm">{user.email}</div>
                                            </div>
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex items-center gap-2 justify-center">
                                            <div
                                                className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-sm font-medium min-w-25 ${
                                                    user.status === "active" ? "text-[#166534] bg-[#DCFCE7]" : user.status === "pending" ? "text-[#EAB308] bg-[#FEF9C3]" : user.status === "suspended" ? "text-[#991B1B] bg-[#FECACA]" : user.status === "rejected" ? "text-[#6B7280] bg-[#E5E7EB]" : "text-[#6B7280] bg-[#E5E7EB]"
                                                }`}
                                            >
                                                <span className={`w-2 h-2 rounded-full mr-2 ${user.status === "active" ? "bg-[#22C55E]" : user.status === "pending" ? "bg-[#EAB308]" : user.status === "suspended" ? "bg-[#EF4444]" : user.status === "rejected" ? "bg-[#6B7280]" : "bg-[#6B7280]"}`}></span>
                                                {user.status.charAt(0).toUpperCase() + user.status.slice(1)}
                                            </div>
                                                ...
                                                <DropdownMenuContent align="end" className="w-48 border-[#E2E8F0] shadow-[0px_1px_16px_0px_rgba(0,0,0,0.08)]">
                                                    <div className="flex flex-col p-2 gap-3">
                                                        ...
                                                        {user.status !== "suspended" && (
                                                            <DropdownMenuItem onClick={() => handleUpdateStatus(user._id, "suspended")} disabled={isUpdatingStatus || isDeleting} className="flex items-center justify-center rounded-full px-4 py-2 text-[#991B1B] bg-[#FECACA] font-medium text-sm data-highlighted:bg-[#FECACA] cursor-pointer">
                                                                <span className="w-2 h-2 bg-[#EF4444] rounded-full"></span>
                                                                Suspend
                                                            </DropdownMenuItem>
                                                        )}
                ...
                <div className="">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-0">
                            ...
                            <Button variant="outline" size="icon" onClick={() => setCurrentPage((prev) => Math.min(prev + 1, pagination.totalPage))} disabled={currentPage === pagination.totalPage} className="border-[#FFEFEB] bg-white text-[#64748B] hover:text-[#FF5A36] hover:bg-[#FFEFEB] rounded-l-none">
                                <ChevronRight className="h-4 w-4" />
                            </Button>
                        </div>
                    </div>
