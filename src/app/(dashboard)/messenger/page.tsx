...
//     // Auto-select first chat on load - Fixed version
//     useEffect(() => {
//         // Only run once when chatsData is loaded and we haven't initialized yet
//         if (chatsData?.data && chatsData.data.length > 0 && !hasInitialized) {
...
//         }
//     }, [chatsData, hasInitialized]);

//     // Alternative: Use a ref to track initialization
...
//                 toast.error(`The following file types are not allowed: ${invalidFiles.map((f) => f.type).join(", ")}\n\nAllowed types: Images (JPEG, PNG, JPG), PDF, and Word documents.`);
//             }
...
//                 return {
//                     isValid: false,
//                     error: `File "${file.name}" has an invalid type. Allowed types: Images (JPEG, PNG, JPG), PDF, and Word documents.`,
//                 };
//             }
//         }
//         return { isValid: true };
...
//                                                             />
//                                                         ) : null}
//                                                         <div className={`w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center ${otherUser.profilePicture ? "hidden" : ""}`}>
...
//                                                                         {isImage ? (
//                                                                             <Image src={fileUrl} alt="Attachment" width={200} height={150} className="rounded-lg" />
//                                                                         ) : (
...
//                                     <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center space-x-1">
//                                         <input type="file" ref={fileInputRef} onChange={handleFileSelect} multiple className="hidden" accept=".jpg,.jpeg,.png,.pdf,.doc,.docx,image/jpeg,image/png,image/jpg,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" />
...
const MessengerPage = () => {
    ...
    useEffect(() => {
        ...
        // Listen for admin chat list updates
        socket.on("adminChatListUpdate", () => {
            console.log("Admin chat list updated");
            refetchChats();
        });
