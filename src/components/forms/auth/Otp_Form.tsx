...
const OtpForm = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsSubmitting(true);
        try {
            const response = await verifyOtp({ email, oneTimeCode: Number(enteredOtp) }).unwrap();
            const token = response?.data;

            if (!token) throw new Error("No token received from server");

            toast.success("OTP verified successfully!");
            router.push(`/auth/new-password?email=${encodeURIComponent(email)}&token=${encodeURIComponent(token)}`);
        } catch (err: any) {
    ...
    const handleResend = async () => {
        setIsResending(true);
        try {
            // You can call a resend OTP mutation here if you have it
            console.log("Resend OTP to:", email);
            await new Promise((resolve) => setTimeout(resolve, 1000));
            setOtp(["", "", "", "", "", ""]);
            inputsRef.current[0]?.focus();
